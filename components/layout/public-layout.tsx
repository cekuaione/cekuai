...
export function PublicLayout({ children, className, showFooter = true }: PublicLayoutProps) {
  return (
    <div className="flex min-h-screen flex-col bg-slate-950 text-white">
      ...
      {showFooter ? (
        <footer className="border-t border-border/60 bg-slate-950/80 py-8">
          <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 text-sm text-muted-foreground md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex gap-4">
              <a className="hover:text-white" href="mailto:support@ceku.ai">
                Contact
              </a>
              <span className="hidden md:block">|</span>
              <a className="hover:text-white" href="/auth/signup">
                Join the beta
              </a>
            </div>
          </div>
